...
func (assets *assetManager) handleSiteModelAssetSubstitution(w http.ResponseWriter, r *http.Request) {
	...
	if !target.local {
		...
		if err := assets.identity().client.JSON(r.Context(), http.MethodPost, target.nodeURL, "/router/v1/node/site/model-assets/substitute", request, &response); err != nil {
	...
	assets.substituteLocalModelAsset(w, r, request)
}

func (assets *assetManager) handleNodeModelAssetSubstitution(w http.ResponseWriter, r *http.Request) {
	request, ok := decodeModelAssetSubstitution(w, r)
	if !ok {
		return
	}
...
func (assets *assetManager) substituteLocalModelAsset(w http.ResponseWriter, r *http.Request, request siteapi.ModelAssetSubstitutionRequest) {
	if !request.Confirm || assets.index == nil || assets.downloader == nil || !modelassets.ValidHash(request.SHA256) || !modelassets.ValidHash(request.ExpectedSHA256) || request.SHA256 == request.ExpectedSHA256 {
	...
	_, _, target, err := assets.modelAssetConfigTarget(siteapi.ModelAssetConfigRequest{ID: request.ID, Filename: request.Filename})
	if err != nil {
