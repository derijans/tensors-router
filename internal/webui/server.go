...
func (server *Server) handleAPI(w http.ResponseWriter, r *http.Request) {
	...
	case r.URL.Path == "/api/text-routing-groups" && (r.Method == http.MethodGet || r.Method == http.MethodPost || r.Method == http.MethodDelete):
		server.proxyRouter(w, r, r.Method, "/router/v1/site/text-routing-groups")
