...
.PHONY: test webui-build webui-check build build-router build-webui build-downloader build-linux build-linux-router build-linux-webui build-linux-downloader clean install-user-service uninstall-user-service
...
build: build-router build-webui build-downloader
...
build-linux: build-linux-router build-linux-webui build-linux-downloader
...
clean:
	rm -rf $(DIST_DIR) $(APP_NAME) $(WEBUI_NAME) $(DOWNLOADER_NAME)
