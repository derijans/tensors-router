...
TRA-2026-001 and TRA-2026-007 are operationally active for the default repository. GitHub Pages serves the public TUF repository. Authenticated maintainer API verification showed that `tuf-publication` has exactly one deployment branch policy, `main`, and exactly three named environment secrets for the delegated upstream-targets, snapshot, and timestamp roles. Publication run `31183299695` independently downloaded and hashed the allowlisted upstream assets, constructed and signed timestamp after targets and snapshot, verified the complete output, and atomically published the repository as one Git commit. A second clean go-tuf v2 client fetched the live Pages endpoint with the embedded root and verified all eight signed target manifests.
...
| Dependabot default-branch alerts | Pass | Authenticated API returned 0 open alerts after `x/crypto` v0.52.0, `x/sys` v0.45.0, and `x/term` v0.43.0 reached `main` |
...
| CodeQL code-scanning result | Pass | The current PR result reports no new alerts; malicious-content-type Ollama regressions cover both earlier reflected-XSS findings. The separate AI-findings processor failed service-side and is not represented as passing |
