...
The Routing column opens the lending links of a model: "Image routing" for an image model and "LLM routing" for an LLM, and a model that is both shows both buttons. The dialog has two lists of models on the other nodes. **Lends work to** names the models that may take requests queued for this model. **Borrows work from** names the models whose queued requests this model may take. Each row has its own **load if not loaded** and **reload what was displaced** flags, which apply to whichever side of that link is the helper. The lists are unfiltered, because the models worth linking are usually the ones a name or hash filter would hide. Each candidate is labelled as the same weights or different weights, compared against the anchor by model hash, and linking a candidate with different weights for the first time requires an explicit acknowledgement. LLM candidates that cannot be linked are shown disabled with the reason. Clearing every row removes all links of the model. See [Cluster Routing](Cluster-Routing#backlog-lending) for what a link changes about scheduling.

The Nodes page lists, per node, the **Held requests**: requests of a linked model that the router is holding before passing them to a backend, with their lane, model, and how long they have waited, and requests currently lent to a helper, marked with the helper node and model. Requests of models without links go straight to the backend and appear only under active requests.
